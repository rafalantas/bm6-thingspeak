...
async def get_bm6_data(address):
    ...
    try:
        async with BleakClient(address, timeout=30) as client:
            ...
            # Wait for voltage and soc to be populated or timeout
...
async def collect_and_send_data():
    ...
    if all_data:
        # map: field1..4 = voltages module 1..4, field5..8 = soc module 1..4
        payload = {
            ...
            "field4": all_data[3]["voltage"] if len(all_data) > 3 else None,
            "field5": all_data[0]["soc"] if len(all_data) > 0 else None,
            "field6": all_data[1]["soc"] if len(all_data) > 1 else None,
            "field7": all_data[2]["soc"] if len(all_data) > 2 else None,
            "field8": all_data[3]["soc"] if len(all_data) > 3 else None,
        }
